...
const EnvSchema = z.object({
  ...
  TRUST_JUDGE_CONFIDENCE: z.coerce.number().min(0).max(1).default(0.7),
  JUDGE_BATCH_SIZE: z.coerce.number().int().positive().default(10),
  SHORTLIST_K: z.coerce.number().int().positive().default(10),
  REVERSE_PASS_K: z.coerce.number().int().positive().default(8),
  REVERSE_PASS_MIN_SIMILARITY: z.coerce.number().min(0).max(1).default(0.55),
  REVERSE_PASS_MIN_CONFIDENCE: z.coerce.number().min(0).max(1).default(0.7),
  QTY_RATIO_MIN: z.coerce.number().positive().default(0.1),
  QTY_RATIO_MAX: z.coerce.number().positive().default(3.0),
  JUDGE_MODEL: z.string().default('gpt-4.1-mini'),
